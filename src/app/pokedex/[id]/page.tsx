  ...
  if (detailQuery.isError || !detailQuery.data) {
    return (
      <div className='min-h-screen bg-surface flex flex-col items-center justify-center gap-4'>
        <p className='px-4 py-2 rounded-xl bg-error text-on-error'>
      ...
      <main className='max-w-4xl mx-auto px-6 py-10 flex flex-col gap-8'>
        ...
        <div className='bg-surface-container-lowest rounded-3xl border border-outline-variant/40 p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-8 shadow-[0_8px_24px_rgba(0,0,0,0.03)]'>
          ...
          <div className='flex flex-col gap-5'>
            ...
            <PrimaryButton onClick={() => setShowForm(true)} className='w-fit'>
              {collectionCount > 0
            ...
            {collectionCount > 0 && (
              <p className='text-xs text-on-surface-variant'>
                This Pokemon is already in your collection ({collectionCount}{' '}
                entr{collectionCount > 1 ? 'ies' : 'y'}).
              </p>
            )}
          </div>
        </div>
        ...
        <div className='bg-surface-container-lowest rounded-3xl border border-outline-variant/40 p-6 shadow-[0_8px_24px_rgba(0,0,0,0.03)]'>
          ...
          <div className='flex flex-col gap-3'>
            {pokemon.stats.map((stat) => (
              ...
              >
                <span className='col-span-3 text-xs font-bold text-on-surface-variant'>
                  {stat.name}
                </span>
                <span className='col-span-2 text-sm font-bold text-on-surface'>
                  {stat.value}
                </span>
        ...
        {showForm && (
          ...
          >
            ...
            >
              ...
              <FavoritesForm onSubmit={handleSaveFavorite} />
            </div>
          </div>
        )}
      </main>
