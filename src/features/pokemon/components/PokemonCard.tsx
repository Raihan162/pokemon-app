  ...
  return (
    ...
    >
      ...
      <div className='mb-3 flex items-center justify-between gap-2'>
        ...
        {pokemon.collectionCount && pokemon.collectionCount > 0 && (
          <span className='text-[10px] font-bold uppercase rounded-full px-2.5 py-1 bg-primary text-on-primary'>
            In Collection{' '}
            {pokemon.collectionCount > 1 ? `(${pokemon.collectionCount})` : ''}
          </span>
        )}
      </div>
      ...
      <div
        className={`w-full aspect-square rounded-2xl bg-gradient-to-b ${pokemon.darkBoxClass} relative overflow-hidden flex items-center justify-center p-4 sm:p-5 shadow-inner border border-slate-950/10`}
      >
        ...
        <svg
          viewBox='0 0 100 100'
          fill='currentColor'
          className='absolute w-[120%] h-[120%] text-white/5 pointer-events-none transform -rotate-12 z-0'
        >
      ...
      {/* Name */}
      <h2 className='text-xl sm:text-2xl font-extrabold text-slate-800 text-center mt-4 sm:mt-5 tracking-tight group-hover:text-slate-900 transition-colors'>
        {pokemon.name}
      </h2>
